...
async def main(_, msg):
    await msg.reply(
        "Pilih String session mana yang ingin kamu pilih!
...
async def generate_session(bot, msg, telethon=False):
    ...
    user_id = msg.chat.id
    api_id_msg = await bot.ask(user_id, 'Kirim api kamu`API_ID`', filters=filters.text)
    ...
    try:
        ...
        return
    api_hash_msg = await bot.ask(user_id, 'Kirim api hash kamu`API_HASH`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    api_hash = api_hash_msg.text
    phone_number_msg = await bot.ask(user_id, 'Kirim nomor telegram kamu `PHONE_NUMBER` Kirim dengan kode negara. \nExample : `+620000000`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by @StringEzz".format("TELETHON" if telethon else "PYROGRAM", string_session)
    try:
